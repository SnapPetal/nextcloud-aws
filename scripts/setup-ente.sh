...
if grep -q "ENTE_POSTGRES_USER" "$ENV_FILE" 2>/dev/null; then
    ...
    sed -i "s/^ENTE_JWT_SECRET=.*/ENTE_JWT_SECRET=$JWT_SECRET/" "$ENV_FILE"
    sed -i "s|^ENTE_KEY_ENCRYPTION=.*|ENTE_KEY_ENCRYPTION=$KEY_ENCRYPTION|" "$ENV_FILE"
    sed -i "s|^ENTE_KEY_HASH=.*|ENTE_KEY_HASH=$KEY_HASH|" "$ENV_FILE"
